...
impl CraneliftBackend {
    ...
    /// Compile SSA program by interpreting SSA instructions (alternative approach)
    /// Maps SSA instructions directly to Cranelift IR without hardcoded patterns
    pub fn compile2(&mut self, program: &SsaProgram) -> Result<*const u8, DioError> {
        self.setup_function_signature();
        ...
        if env::var("DIO_DEBUG_JIT").is_ok() {
            println!("--- Cranelift IR (compile2 - SSA-driven) ---");
            println!("{}", self.context.func.display());
            println!("--------------------------------------------");
        }
...
mod tests {
    ...
    #[test]
    fn test_cranelift_compilation_add_compile2() {
        let expr = parse_expr("(lambda ([I64Array a] [I64Array b] I64Array) (+ a b))").unwrap();
        let ssa_program = ast_to_ssa(&expr).unwrap();
        let mut backend = CraneliftBackend::new().unwrap();
        backend.compile2(&ssa_program).unwrap();
    }

    #[test]
    fn test_cranelift_compilation_sub_compile2() {
        let expr = parse_expr("(lambda ([I64Array a] [I64Array b] I64Array) (- a b))").unwrap();
        let ssa_program = ast_to_ssa(&expr).unwrap();
        let mut backend = CraneliftBackend::new().unwrap();
        backend.compile2(&ssa_program).unwrap();
    ...
    #[test]
    fn test_cranelift_compilation_nary_add_compile2() {
        let expr = parse_expr("(lambda ([I64Array a] [I64Array b] [I64Array c] I64Array) (+ a b c))").unwrap();
        let ssa_program = ast_to_ssa(&expr).unwrap();
        let mut backend = CraneliftBackend::new().unwrap();
        backend.compile2(&ssa_program).unwrap();
