...
// =============================================================================
// Owned<T> - Marker for by-value struct parameters
// =============================================================================

/// Marker type for structs passed by value to JIT functions.
///
/// `Var<Owned<T>>` represents a struct that is passed by value at the ABI
/// boundary. Internally, the struct is stored in a stack slot and accessed
/// via pointer, but references to it cannot escape the function.
///
/// This is used to distinguish by-value structs from by-reference structs
/// (`Var<SRef<'a, T>>`) in the type system, enabling correct lifetime tracking.
///
/// # Example
/// ```ignore
/// // By-value: references cannot escape
/// compiler.fun1("process", |_ctx, data: Var<Owned<MyStruct>>| {
///     data.field(MyStructType::inner).get(InnerType::value)
/// });
///
/// // By-reference: references can be returned with input lifetime
/// compiler.fun1("get_ref", |_ctx, data: Var<SRef<'_, MyStruct>>| {
///     data.get_ref(MyStructType::inner)
/// });
/// ```
...
impl<T: StagedType> StagedType for Owned<T> {
    type RuntimeValue = T::RuntimeValue;

    fn cranelift_type() -> cranelift_codegen::ir::Type {
        // Owned structs are represented as pointers to stack slots internally
        types::I64
    }
...
impl<T: CopyType> CopyType for Owned<T> {}
