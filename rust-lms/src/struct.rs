...
/// Project a field from a unique staged mutable-reference variable.
///
/// # Safety
///
/// The returned projection must not overlap another staged access derived
/// from `base` while the generated operation using it executes.
pub unsafe fn field_addr_mut_unchecked<'a, T, F>(
    base: &mut Var<SRefMut<'a, T>>,
    _field: F,
) -> FieldAddr<VarUse<SRefMut<'a, T>>, F>
where
    T: StagedType + 'a,
    F: Field<Parent = T>,
{
