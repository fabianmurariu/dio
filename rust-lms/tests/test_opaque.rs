...
fn opaque_ref_param_round_trips() {
    ...
    let f = compiler.fun1("count_nodes", move |_ctx, g: Var<SRef<Opaque<Graph>>>| {
        call_extern1::<_, _, SRef<Opaque<Graph>>, U64Type>(node_count, g)
    });
    ...
    let f = compiler.fun1(
        ...
        move |ctx, g: Var<SRefMut<Opaque<Graph>>>| {
            ctx.emit(call_extern2::<
                ...
                _,
                _,
                SRefMut<Opaque<Graph>>,
                U64Type,
                UnitType,
            >(push, g, Const::<U64Type>::new(99)));
            call_extern1::<_, _, SRefMut<Opaque<Graph>>, U64Type>(node_count, g)
        },
    );
    let kernel = compiler.compile(f).expect("compile").as_fn();
