...
fn test_ergonomic_arithmetic() {
    ...
    let expr = (
        assign(x, 10i64),                    // Ergonomic assign
        add::<I64Type, _, _>(x, 5i64)        // Ergonomic add
    );
...
fn test_ergonomic_let_var() {
    ...
    // let_var accepts primitives directly
...
fn test_ergonomic_while_loop() {
    ...
    let count_to_n = compiler.fun1("count_to_n", |n: Var<I64Type>| {
        (
            assign(i, 0i64),                  // Ergonomic assign
            assign(sum, 0i64),
            while_loop(
...
fn test_ergonomic_mixed_operations() {
    ...
    let (x, x_init) = compiler.let_var(5i64);
    let (y, y_init) = compiler.let_var(10i64);

    // Mix of all ergonomic features
    let expr = (
        x_init,
        y_init,
        if_then_else(
