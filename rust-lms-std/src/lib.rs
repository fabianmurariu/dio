...
//! [`rust_lms`]. The "Data Structures" layer of a Tidy-Tuples-style stack (see
//! `docs/path_to_umbra_group_by.md`): growable, host-backed structures a kernel
//! folds into, reused by GROUP BY, joins, sort, and `DISTINCT`.
//!
//! **Phase 1: [`SVec`] — a growable dynamic array.** It proves the one idea the
//! whole toolbox rests on: a kernel structure can *grow* without dangling the
//! pointer baked into the kernel. The trick is **handle indirection** — we bake a
//! pointer to a stable [`RawVec`] *control block*, never to the (movable) buffer.
//! Every access reloads the buffer pointer from the block; growth reallocs and
//! writes the new pointer back into the block. The block never moves, so its baked
//! address stays valid for the whole run.
//!
//! Split of work (Umbra's proxy model): the hot path — index, load, store, the
//! `len == cap` check — is **generated** staged code; only the cold path (realloc)
//! is an [`svec_grow`] extern call.
