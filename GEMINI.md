...
To inspect the Intermediate Representation (IR) generated by the Cranelift backend, you can set the `DIO_DEBUG_JIT` environment variable. When this variable is set, the backend will pretty-print the Cranelift IR for each function it compiles to standard output.

**Example:**

```bash
DIO_DEBUG_JIT=1 cargo test -- --nocapture
DIO_DEBUG_PIPELINE=1 cargo test <test-name> -- --nocapture
DIO_DEBUG_PIPELINE=1 cargo test -- --nocapture
```
